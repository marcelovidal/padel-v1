...
import { submitTournamentPlayoffMatchResultAction } from "@/lib/actions/tournaments.actions";
...
export function TournamentPlayoffResultForm({
  tournamentId,
  playoffMatchId,
  canSubmit,
  hasResult,
    ...
    <div className="mt-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
      ...
      {!canSubmit ? (
        <p className="mt-1 text-[11px] text-amber-700">
          Disponible cuando el partido este finalizado y con equipos definidos.
        </p>
      ) : null}

      {open && canSubmit ? (
        <form action={submitTournamentPlayoffMatchResultAction} className="mt-2 space-y-2">
