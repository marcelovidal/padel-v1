...
import { scheduleLeagueMatchAction } from "@/lib/actions/leagues.actions";
...
export function LeagueMatchScheduleForm({
  leagueId,
  leagueMatchId,
  courts,
  defaultDate,
  ...
  return (
    <form action={scheduleLeagueMatchAction} className="mt-2 grid gap-2 md:grid-cols-5">
      <input type="hidden" name="league_id" value={leagueId} />
      <input type="hidden" name="league_match_id" value={leagueMatchId} />
      <select
        name="court_id"
        value={courtId}
      ...
      <div className="md:col-span-2">
        ...
        {selectedCourt ? (
          <p className="mt-1 text-[11px] text-gray-500">
            Slots validos: {selectedCourt.opening_time.slice(0, 5)}-{selectedCourt.closing_time.slice(0, 5)} cada{" "}
            {slotInterval} min.
          </p>
        ) : (
