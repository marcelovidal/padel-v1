    ...
    <div className="rounded-[14px] border border-[var(--border-soft)] bg-[var(--bg-card)] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.07)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="flex h-[22px] w-[22px] items-center justify-center rounded-[6px] bg-brand-amarillo/20">
            <svg className="h-3 w-3 text-brand-amarillo" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
          ...
          <h2 className="text-[11px] font-black uppercase tracking-[0.16em] text-brand-gris-mid">Logros</h2>
        </div>
        ...
        {BADGE_CATALOG.map((def) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col items-center gap-1.5 rounded-xl border p-3 transition-opacity ${
                unlocked ? def.color : "border-gray-100 bg-gray-50 opacity-35"
              }`}
            >
              <span className="font-display text-[22px] font-black leading-none text-gray-700">{def.icon}</span>
