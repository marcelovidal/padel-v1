...
export default async function ClubPublicLeaguesPage({
  ...
  // Se mantiene el gate de sesion que tenia /club/[clubId]/leagues. Abrir las
  // ligas al publico anonimo es un cambio de alcance, no una mudanza de ruta.
  if (!user) {
  ...
  if (matchedBy === "id" && club.slug) {
    redirect(`${publicClubHref(club)}/ligas`);
  }

  const service = new LeaguesService();
  const leagues = await service.listPublicClubLeagues(club.id);
  ...
  return (
    <div className="container mx-auto max-w-5xl space-y-6 p-4">
      ...
      <div>
        <Link
          href={publicClubHref(club)}
          className="text-sm font-semibold text-blue-700 hover:underline"
        >
          Volver al perfil del club
        </Link>
      </div>
    </div>
  );
