...
export default async function MiClubLeagueDetailPage({
  ...
  // El gate de propiedad va antes de cualquier otra carga: en el original
  // corria despues de listDivisions() y getLeagueRegistrations().
  const league = await leaguesService.getLeagueById(leagueId).catch(() => null);
  if (!league || league.club_id !== club.id) {
    return notFound();
  }

  const [divisions, players, courts, bookingSettings] = await Promise.all([
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {divisionData.map(({ division, teams, groupData, playoffMatches }) => {
        ...
        // FRAGIL: el reparto de partidos por division se hace en JS contra un join
        // anidado del select (league_groups -> league_divisions). Si cambia la forma
        // del select en listLeagueMatches(), esto devuelve [] en silencio y la
        // division aparece sin fixture. Portado tal cual del original.
