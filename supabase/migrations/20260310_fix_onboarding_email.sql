-- Fix: add p_email parameter to player_complete_onboarding
-- so the user's auth email is saved to players.email during onboarding.
-- Preserves all logic from 20260217_stage_n_onboarding_lock (ONBOARDING_ALREADY_COMPLETED guard,
-- onboarding_completed_at, onboarding_version).

-- Drop existing 13-param signature to replace with 14-param version
DROP FUNCTION IF EXISTS public.player_complete_onboarding(text, text, text, text, player_position, int, text, text, text, text, text, int, text);
...
BEGIN
  ...
  -- Find existing player and onboarding state
...
GRANT EXECUTE ON FUNCTION public.player_complete_onboarding TO authenticated;
